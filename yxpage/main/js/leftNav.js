...
function cus_navBar(strData) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (data[i].children != undefined && data[i].children.length > 0) {
			ulHtml += '<a class="" href="javascript:;"><i class="layui-icon aa" data-icon="' + data[i].icon + '">&#xe624;</i><cite>' + data[i].title +'</cite>'+(data[i].wait_tip_service !== null ? '<span class="layui-badge">' + data[i].wait_tip_service +'</span>' : '' )+'<span class="layui-nav-more"></span></a>'
			//二级菜单循环
			var secondData = data[i].children;
				ulHtml += '<dl class="layui-nav-child">';
			for (var j = 0; j < secondData.length; j++) {
				if (secondData[j].children != undefined && secondData[j].children.length > 0) {
					ulHtml += '<dd>';
					ulHtml += '<a href="javascript:;" class="menu_three "><i class="layui-icon" data-icon="' + '&#xe631;' + '">&#xe631;</i><cite>' + secondData[j].title + '</cite><span class="three_down_arrow"></span></a>';
					//三级菜单循环
					var thirdData = secondData[j].children;
					ulHtml += '<ol class="layui-nav-child" style="display: none;">';
					for (var k = 0; k < thirdData.length; k++) {
