...
layui.config({
    ...
    crosAjax(path, "POST", bxReq, function (backdata) {
        if (backdata.data != undefined) {
            ...
            // 保存 app 数据到本地
            window.sessionStorage.setItem('sys_apps',JSON.stringify(backdata.data) )
            ...
            if (view_mode && "customize" == view_mode) {
                ...
                if (backdata.data.length > 0) {
                    ...
                    var i = 0;
                    var init_app_no = "";
                    // window.sessionStorage.setItem('applications', JSON.stringify(backdata.data) ) // 保存应
                    $.each(backdata.data, function (index, value) {
                        if (i == 0) {
                            init_app_no = value.app_no;
                            i++;
                        }
                        if (backdata.data.length > 1) {
                            htmlStr += '<li class="" style="margin:0px 0px 0px 0px">';
                            htmlStr += '<a href="javascript:tab.render(\'' + value.app_no + '\');" id="app_tip_' + value.app_no + '" class="">';
                            htmlStr += '<img  class="img-responsive" src="' + value.app_icon + '">';
                            htmlStr += '<h6 class="">' + value.app_name + '</h6>';
                            htmlStr += '</a>';
                            htmlStr += '</li>';
                        }
                    });
            ...
            // 保存 app 数据到本地
            window.sessionStorage.setItem('sys_apps',JSON.stringify([]) )
        }
    });
    var form = layui.form(),
        layer = layui.layer,
    ...
    // 添加新窗口
    document.querySelectorAll('.layui-nav .layui-nav-item a').forEach(item=>{
        if(item.dataset.url){
    ...
    $("body").on("click", ".layui-nav .layui-nav-item a", function (e) {
        // let href = $(this).context.dataset.url
        let href = e.target.dataset.url
        //如果不存在子级
        if ($(this).siblings().length == 0) {
            var id = $(this).attr("id");
            // console.log($(this).context)
            if(id == 'navsHome'){
                $("#top_tabs > li:first").click()
                document.getElementById('mainpage').contentWindow.location.reload(true);
            }
...
function showHome() {
    ...
    // $(".top_tabs").css('display','none');
    //
    $(".layui-layout-admin").toggleClass("showMenu");
    $("#menu-hide-icon").toggleClass("fa-outdent fa-indent");
//渲染顶部窗口
    // tab.tabMove();
    sessionStorage.setItem('show-home-layout',true)
    $("#navsHome.layui-nav-item").toggleClass("active"); // 系统首页选中效果

    $('#mainpage').attr('src', 'page/home.html');
