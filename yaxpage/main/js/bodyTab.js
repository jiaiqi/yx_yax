...
layui.define(["element", "jquery"], function (exports) {
  ...
  // 菜单搜索：生成搜索框 HTML
  function getMenuSearchHTML() {
    return (
      '<div class="menu-search">'
      + '<div class="menu-search-row">'
      + '<div class="menu-search-input-wrap">'
      + '<input type="text" id="menu-search-input" class="layui-input" autocomplete="off" placeholder="搜索菜单..." />'
      + '<span id="menu-search-clear" class="menu-search-clear" title="清空">×</span>'
      + '</div>'
      + '</div>'
      + '</div>'
    );
  ...
  // 判断是否显示菜单搜索框：基于 sessionStorage.pages_attribute['菜单显示搜索框']
  function isMenuSearchEnabled() {
    try {
      var obj = null;
      var raw = null;
      if (typeof sessionStorage !== 'undefined') {
        raw = sessionStorage.getItem('pages_attribute');
      }
      if (raw) {
        try { obj = JSON.parse(raw); } catch (e) { obj = null; }
      } else if (sessionStorage && sessionStorage.pages_attribute) {
        var direct = sessionStorage.pages_attribute;
        if (typeof direct === 'string') {
          try { obj = JSON.parse(direct); } catch (e2) { obj = null; }
        } else if (typeof direct === 'object') {
          obj = direct;
        }
      }
      if (obj && Object.prototype.hasOwnProperty.call(obj, '菜单显示搜索框')) {
        var v = obj['菜单显示搜索框'];
        if (v === false) return false;
        if (typeof v === 'string') {
          var s = v.trim().toLowerCase();
          if (s === '否' || s === 'false') return false;
        }
      }
    } catch (err) { }
    return true; // 默认显示
  ...
  // 初始化每个 cite 的原始文本，便于反复高亮/还原
  function initRawTitles($tree) {
    $tree.find('cite').each(function () {
      var $cite = $(this);
      if ($cite.data('rawTitle') == null) {
        $cite.data('rawTitle', $cite.text());
      }
    });
  }

  // 菜单搜索： 应用高亮
  function applyHighlight($cite, q) {
    var raw = $cite.data('rawTitle');
    if (raw == null) raw = $cite.text();
    if (!q) {
      $cite.text(raw);
      return;
    }
    var reg = new RegExp(escapeRegExp(q), 'ig');
    var html = raw.replace(reg, function (m) { return '<i class="menu-highlight">' + m + '</i>'; });
    $cite.html(html);
  }
  ...
  function clearAllHighlights($tree) {
    $tree.find('cite').each(function () {
      var $cite = $(this);
      var raw = $cite.data('rawTitle');
      if (raw != null) $cite.html(raw);
    });
  }

  // 菜单搜索：执行过滤
  function filterSideMenu(term) {
    var q = (term || '').toLowerCase().trim();
    var $tree = $(".navBar .layui-nav.layui-nav-tree");
    if (!$tree.length) return;
    initRawTitles($tree);

    // 当查询为空时，重置显示
    if (!q) {
      $tree.find("li.layui-nav-item").show();
      $tree.find("dl.layui-nav-child dd").show();
      $tree.find("ol.layui-nav-child li").show();
      clearAllHighlights($tree);
      // 折叠恢复默认（不强制折叠，保留当前展开状态）
      return;
    }

    $tree.find("li.layui-nav-item").each(function () {
      var $top = $(this);
      var $topCite = $top.find("> a > cite");
      var topTitle = ($topCite.data('rawTitle') || $topCite.text() || '').toLowerCase();
      var topMatch = topTitle.indexOf(q) !== -1;
      applyHighlight($topCite, q);
      var visibleChildren = 0;
      var hasChild = $top.find('> dl.layui-nav-child').length > 0;

      // 二级菜单（包含可展开 和 直接链接 两种）
      $top.find("dl.layui-nav-child > dd").each(function () {
        var $dd = $(this);
        var isExpandable = $dd.find("> a.menu_three").length > 0;
        if (isExpandable) {
          var $secCite = $dd.find("> a.menu_three > cite");
          var secondTitle = ($secCite.data('rawTitle') || $secCite.text() || '').toLowerCase();
          var secondMatch = secondTitle.indexOf(q) !== -1;
          applyHighlight($secCite, q);
          var thirdVisible = 0;
          $dd.find("ol.layui-nav-child li").each(function () {
            var $li = $(this);
            var $thirdCite = $li.find("a > cite");
            var thirdTitle = ($thirdCite.data('rawTitle') || $thirdCite.text() || '').toLowerCase();
            var thirdMatch = thirdTitle.indexOf(q) !== -1;
            applyHighlight($thirdCite, q);
            $li.toggle(thirdMatch);
            if (thirdMatch) thirdVisible++;
          });
          var ddVisible = (secondMatch || thirdVisible > 0);
          $dd.toggle(ddVisible);
          if (ddVisible) {
            visibleChildren++;
            // 展开匹配的三级菜单
            $dd.find("> a.menu_three span").removeClass("layui-nav-more").addClass("three_down_arrow");
            $dd.find("ol.layui-nav-child").show();
          } else {
            // 若不可见，折叠
            $dd.find("> a.menu_three span").removeClass("three_down_arrow").addClass("layui-nav-more");
            $dd.find("ol.layui-nav-child").hide();
          }
        } else {
          // 二级直接链接
          var $secLinkCite = $dd.find("> a > cite");
          var secondTitleLink = ($secLinkCite.data('rawTitle') || $secLinkCite.text() || '').toLowerCase();
          var secondLinkMatch = secondTitleLink.indexOf(q) !== -1;
          applyHighlight($secLinkCite, q);
          $dd.toggle(secondLinkMatch);
          if (secondLinkMatch) visibleChildren++;
        }
      });

      if (hasChild) {
        $top.toggle(topMatch || visibleChildren > 0);
      } else {
        // 顶级叶子
        $top.toggle(topMatch);
      }
  ...
  function bindMenuSearch() {
    ...
    $input.off("input.menuSearch keyup.menuSearch");
    $input.on("input.menuSearch keyup.menuSearch", function () {
      var v = this.value || '';
      filterSideMenu(v);
      // 根据是否有内容，显示/隐藏叉叉
      var hasVal = v.trim().length > 0;
      $("#menu-search-clear").toggle(hasVal);
    });
    $("#menu-search-clear").off('click.menuSearchClear');
    $("#menu-search-clear").on('click.menuSearchClear', function () {
      $input.val('');
      filterSideMenu('');
      $(this).hide();
      $input.focus();
    ...
    if (resData.length > 0) {
      ...
      crosAjax(path, "POST", requestData, function (backdata) {
        if (backdata.data == undefined) {
          ...
          var __searchPrefix = isMenuSearchEnabled() ? getMenuSearchHTML() : '';
          $(".navBar").html(__searchPrefix + cus_navBar(data)).height($(window).height() - 60);
          element.init();
          if (isMenuSearchEnabled()) bindMenuSearch();
  ...
  Tab.prototype.cus_render = function (menu_no) {
    ...
    crosAjax(path, "POST", requestData, function (backdata) {
      if (backdata.data == undefined) {
        ...
        var _this = this;
        var __searchPrefix2 = isMenuSearchEnabled() ? getMenuSearchHTML() : '';
        $(".navBar").html(__searchPrefix2 + cus_navBar(data)).height($(window).height() - 60);
        element.init();
        if (isMenuSearchEnabled()) bindMenuSearch();
        $(window).resize(function () {
          $(".navBar").height($(window).height() - 60);
    ...
    if (app_no != undefined) {
      ...
      crosAjax(path, "POST", requestData, function (backdata) {
        if (backdata.data == undefined) {
          ...
          var _this = this;
          var __searchPrefix3 = isMenuSearchEnabled() ? getMenuSearchHTML() : '';
          $(".navBar").html(__searchPrefix3 + navBar(data, app_no)).height($(window).height() - 60);
          element.init();
          if (isMenuSearchEnabled()) bindMenuSearch();
          $(window).resize(function () {
            $(".navBar").height($(window).height() - 60);
  ...
  Tab.prototype.tabAdd = function (_this) {
    ...
    if (_this.attr('data-url')?.includes('target=_self') || _this.attr('data-url')?.includes('target=_self')) {
      ...
      if (that.hasTab(_this.attr("data-url")) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
        ...
        tabIdIndex++;
        // 获取标题文本并进行长度检查，超过20个字符时省略
        var titleText = _this.find("cite").text();
        if (titleText.length > 20) {
          titleText = titleText.substring(0, 20) + '...';
        }
        title += '<cite title="' + _this.find("cite").text() + '">' + titleText + '</cite><span style="display:none">' + _this.attr("data-url") + '</span>';
  ...
  Tab.prototype.addTab = function (page) {
    ...
    if (that.hasTab(page.url) == -1) {
      ...
      title += '<cite title="' + page.title + '">' + displayTitle + '</cite><span style="display:none">' + page.url + '</span>';
